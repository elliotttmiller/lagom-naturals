...
const ALL_TABS = [
  ...
  { id:'sales',         label:'Sales',         icon:'        label:'Events',        icon:'◆' },
        ...
        <div className="detail-panel fade">
          ...
          <div style={{padding:20,flex:1,overflowY:'auto'}}>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,marginBottom:10}}>
              <div style={{fontWeight:800,fontSize:14}}>Account 360 · Sales</div>
              <button className="btn btn-g btn-sm" onClick={()=>go&&go('sales')}>Open Sales</button>
            </div>
            <div className="g2" style={{marginBottom:14,gap:8}}>
              {[
                ['Lifetime Revenue',fmtFull$(sales?.lifetime_revenue||0),P.teal],
                ['Lifetime Cases',Number(sales?.lifetime_cases||0).toFixed(Number(sales?.lifetime_cases||0)%1?1:0),P.plum],
                ['Last Order',sales?.last_order_date?fmtDS(sales.last_order_date):'-',P.amber],
                ['Balance Due',fmtFull$(sales?.balance_due||0),Number(sales?.balance_due||0)>0?P.rose:P.teal],
              ].map(([l,v,c])=><div key={l} style={{padding:10,background:P.slateL,borderRadius:9}}><div style={{fontSize:9.5,color:P.t3,fontWeight:800,textTransform:'uppercase'}}>{l}</div><div style={{fontSize:14,fontWeight:900,color:c,marginTop:3}}>{v}</div></div>)}
            </div>
            ...
            <div style={{border:`1px solid ${P.border}`,borderRadius:9,overflow:'hidden',marginBottom:14}}>
              {catalog.length?catalog.map((prod,i)=>{
                const placed=placements.find(x=>x.product_id===prod.id||x.sku===prod.sku);
                return <div key={prod.id} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',borderBottom:i<catalog.length-1?`1px solid ${P.border}`:'none'}}>
                  <div style={{width:7,height:7,borderRadius:'50%',background:placed?P.teal:P.border,flexShrink:0}}/>
                  <div style={{flex:1,minWidth:0}}><div style={{fontSize:11.5,fontWeight:650,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{prod.name}</div>{placed&&<div style={{fontSize:10,color:P.t3}}>{Number(placed.lifetime_cases||0)} cases · last {fmtDS(placed.last_order_date)}</div>}</div>
                  <Badge label={placed?'Placed':'Opportunity'} color={placed?P.teal:P.t3}/>
                </div>;
              }):<div style={{fontSize:12,color:P.t3,padding:10}}>No catalog data</div>}
            </div>
            <div style={{fontWeight:750,fontSize:12.5,marginBottom:7}}>Recent Invoices</div>
            {accountInvoices.length?accountInvoices.map(inv=><div key={inv.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8,padding:'8px 0',borderBottom:`1px solid ${P.border}`}}>
              <div><div style={{fontFamily:'monospace',fontSize:11,fontWeight:800,color:P.teal}}>{inv.invoice_number}</div><div style={{fontSize:10.5,color:P.t3}}>{fmtDS(inv.issued_at)} · {inv.sale_type||'Sale'}</div></div>
              <div style={{textAlign:'right'}}><div style={{fontSize:12,fontWeight:800}}>{fmtFull$(inv.invoice_total)}</div>{Number(inv.balance_due||0)>0&&<div style={{fontSize:10,color:P.rose}}>{fmtFull$(inv.balance_due)} due</div>}</div>
            </div>):<div style={{fontSize:12,color:P.t3,marginBottom:12}}>No sales history yet</div>}
            <div className="sep"/>
